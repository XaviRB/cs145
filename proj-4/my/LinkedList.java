...
public class LinkedList {
    ...
    //This is a copy method that copies the list of
    //coords and it appends the old list into the new one
    //will need it to make sure that the right path is coppied
    public static <T> List<T> copy(List<T> list){

	List<T> newList     = new List<T>();
	Node<T> coord = list.head;

	while(coord != null){
		append(newList, coord.datum);
		coord = coord.next;
	}
    ...
    //This method checks if the coords are equal to the data in item
    //and if so then it will return true or false. This will be helpful
    //to determine if we need to keep looking for another path.
    public static <T> boolean checker(List<T> list, T item){
	assert list != null;
